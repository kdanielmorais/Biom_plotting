...
if (g == "BIOM") {
  ...
  taxonomy[,1] <- gsub("No blast hit", "", taxonomy[,1])
  v <- 0
  for (v in 1:7) {
  ...
  }else 
  {
    ...
    pdf(file = paste("legend_sample_",opt$out,".pdf",sep = ""), width = 17, height = 15)
  ...
  }else 
  {
    ...
    dev.off()
     ...
     #plotting legend isolated
    pdf(file = paste("legend_group_",opt$out,".pdf",sep = ""), width = 17, height = 15)
